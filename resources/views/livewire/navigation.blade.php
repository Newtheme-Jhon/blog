<nav x-data="{ open: false }" class="bg-gray-800">
    <div class="mx-auto max-w-7xl px-2 sm:px-6 lg:px-8">
      <div class="relative flex h-16 items-center justify-between">
        ...
        <!-- Mobile menu button-->
        <div @click="open = ! open" class="absolute inset-y-0 left-0 flex items-center sm:hidden">
          ...
          <div class="hidden sm:ml-6 sm:block">
            <div class="flex space-x-4">
              ...
              @foreach ($categories as $category)
                <a href="{{route('posts.category', $category)}}" class="text-gray-300 hover:bg-gray-700 hover:text-white rounded-md px-3 py-2 text-sm font-medium">{{$category->nombre}}</a>
              @endforeach
    ...
    <div x-show="open" @click.outside="open = false" class="sm:hidden" id="mobile-menu">
      <div class="space-y-1 px-2 pt-2 pb-3">
        ...
        @foreach ($categories as $category)
          <a href="{{route('posts.category', $category)}}" class="text-gray-300 hover:bg-gray-700 hover:text-white block rounded-md px-3 py-2 text-base font-medium">{{$category->nombre}}</a>
        @endforeach
